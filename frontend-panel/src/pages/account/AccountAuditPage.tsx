...
export default function AccountAuditPage() {
	...
	usePageTitle(t("account.auditPage.title"));
	...
	const entityTypeOptions = [
		{
			label: t("account.auditPage.filters.allTypes"),
			value: "__all__",
		},
		...
		>
			<div className="relative min-w-[14rem] flex-1 md:max-w-sm">
				...
				<Input
					...
					placeholder={t("account.auditPage.filters.actionPlaceholder")}
					className="pl-9"
				/>
			</div>
			<Select
			...
			>
				<SelectTrigger
					width="compact"
					aria-label={t("account.auditPage.entity")}
				>
					<SelectValue />
				</SelectTrigger>
	...
	return (
		<AdminPageShell>
			<AdminPageHeader
				title={t("account.auditPage.title")}
				description={t("account.auditPage.description")}
				actions={
					<Button
						type="button"
						variant="outline"
			...
			{error && items.length === 0 ? (
				<AdminSurface padded={false}>
					<EmptyState
						...
						title={t("account.auditPage.loadErrorTitle")}
						description={error}
						action={
							<Button
								type="button"
				...
				<AdminTableList
					columns={5}
					emptyDescription={t("account.auditPage.emptyDescription")}
					emptyIcon={emptyIcon}
					emptyTitle={t("account.auditPage.emptyTitle")}
					filtered={hasServerFilters}
					filteredEmptyAction={filteredEmptyAction}
					filteredEmptyDescription={t(
						"account.auditPage.filteredEmptyDescription",
					)}
					filteredEmptyTitle={t("account.auditPage.filteredEmptyTitle")}
					headerRow={headerRow}
					items={items}
					loading={loading}
