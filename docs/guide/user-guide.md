...
这一页按真实使用流程写。你不需要先读完整个 Yggdrasil 协议；先知道账号、角色名、皮肤和启动器登录分别在哪里发挥作用就够了。

如果你是部署者或服主，建议也先看一遍普通用户流程。很多接入问题不是协议坏了，而是 profile 没创建、公开 URL 没配好、或者启动器还拿着旧 token。
...
Minecraft profile 才是你进服时看到的玩家身份。它包含协议里的 `id` 和 `name`，也会被用于 skin/cape 的 textures property。
