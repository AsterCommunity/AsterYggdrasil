...
mod tests {
    ...
    fn sqlite_urls_without_query_default_to_rwc_mode() {
        assert_eq!(
            normalize_database_url("sqlite:///var/lib/asteryggdrasil/app.db"),
            "sqlite:///var/lib/asteryggdrasil/app.db?mode=rwc"
        );
        assert_eq!(
            normalize_database_url("sqlite://data/asteryggdrasil.db"),
    ...
    fn sqlite_memory_and_existing_queries_are_preserved() {
        ...
        assert_eq!(
            normalize_database_url("sqlite:///var/lib/asteryggdrasil/app.db?mode=ro"),
            "sqlite:///var/lib/asteryggdrasil/app.db?mode=ro"
        );
        assert_eq!(
            normalize_database_url("postgres://user:pass@localhost/asteryggdrasil"),
            "postgres://user:pass@localhost/asteryggdrasil"
        );
    }
    ...
    fn sqlite_reader_url_forces_read_only_mode() {
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asteryggdrasil/app.db?mode=rwc"),
            "sqlite:///var/lib/asteryggdrasil/app.db?mode=ro"
        );
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asteryggdrasil/app.db?cache=shared"),
            "sqlite:///var/lib/asteryggdrasil/app.db?mode=ro&cache=shared"
        );
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asteryggdrasil/app.db?"),
            "sqlite:///var/lib/asteryggdrasil/app.db?mode=ro"
        );
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asteryggdrasil/app.db?&cache=shared"),
    ...
    async fn sqlite_reader_pool_is_query_only() {
        let url = format!(
            "sqlite:///tmp/asteryggdrasil-reader-pool-{}.db?mode=rwc",
            uuid::Uuid::new_v4()
        );
        let cfg = DatabaseConfig {
            url,
    ...
    async fn sqlite_reader_pool_reads_while_writer_connection_is_busy() {
        let url = format!(
            "sqlite:///tmp/asteryggdrasil-reader-writer-split-{}.db?mode=rwc",
            uuid::Uuid::new_v4()
        );
        let cfg = DatabaseConfig {
            url,
